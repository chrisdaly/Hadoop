...
def mapper():
    ...
    for line in reader:
        # Only interested in the 5th line (the body field).
        body = line[4]

        # Count the number of punctuation marks in the line.
        contains = body.count('.') + body.count('!') + body.count('?')

        # Check if there are no punctuation marks.
        if contains == 0:
            writer.writerow(line)

        # If there is a punctuation mark, then check if it's at the very end.
        if contains == 1:
